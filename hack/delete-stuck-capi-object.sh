...
kubectl patch crd/clusters.cluster.x-k8s.io -p  '{"metadata":{"finalizers":[]}}' --type=merge -n $NAMESPACE
...
kubectl patch crd/openstackmachinetemplates.infrastructure.cluster.x-k8s.io -p  '{"metadata":{"finalizers":[]}}' --type=merge -n $NAMESPACE