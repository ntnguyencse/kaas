...
func WatchGrpcEndpoint(args ...string) {
	...
	for i, arg := range args {
		...
		case "format":
			if s[1] == "summary" {
				...
				err := fmt.Errorf("Invalid output format parameter: %s\n", s[1])
				fmt.Println("Use: 'emcoctl watch --help'")
				fmt.Println("Error:", err)
				return
			}
	...
	s := strings.Split(anchor, "/")
	if len(s) < 1 {
		err := fmt.Errorf("Invalid Anchor: %s\n", s)
		fmt.Println("Use: 'emcoctl watch --help'")
		fmt.Println("Error:", err)
		return
	}
	...
	case "projects":
		...
		err := fmt.Errorf("Invalid status anchor: %s\n", s)
		fmt.Println("Use: 'emcoctl watch --help'")
		fmt.Println("Error:", err)
		return
	default:
	...
	if err != nil {
		errmsg := fmt.Errorf("Error connecting to gRPC status endpoint: %s, Error: %s\n", endpoint, err)
		fmt.Println("Use: 'emcoctl watch --help'")
	...
	if err != nil {
		err := fmt.Errorf("Error registering for status notifications, gRPC status endpoint: %s, Error: %s\n", endpoint, err)
		fmt.Println("Use: 'emcoctl watch --help'")
		fmt.Println("Error:", err)
		return
	}
	...
	for true {
		...
		if err != nil {
			err := fmt.Errorf("error reading from status stream: %s\n", err)
			fmt.Println("Error:", err)
